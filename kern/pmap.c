...
boot_alloc(uint32_t n)
{
    ...
    if (n>0) {
        ...
        if (nextfree > (char *)(0x7ff000+KERNTOP)) {
            panic("Out of memory in pmap.c:boot_alloc()");
        }
    }
...
page_init(void)
{
    ...
    int extend = (int)ROUNDUP( ((char*)pages)
...
page_free(struct PageInfo *pp)
{
    ...
    if (pp->pp_ref != 0) {
        panic("Out of memory in pmap.c:page_free()");
    }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    if(*pgdir_entry & PTE_P) {
        ...
        if((pp = page_alloc(ALLOC_ZERO)) == NULL)
            return NULL;
        pp->pp_ref = 1;
        pgtable = (pte_t*)KADDR(page2pa(pp));
...
void meminfo(void)
{
    ...
    for (i = 0; i < npages; i++) {
        if (i==0) {
            if (pages[0].pp_link != 0) {
                select = 1;
            }
            cprintf("%d", i+1);
            continue;
        }
        if ((pages[i].pp_link == pages[i-1].pp_link && pages[i-1].pp_link == NULL) ||
            (pages[i].pp_link != NULL && pages[i-1].pp_link != NULL))
            goto N;
